...
// scripts/plugins/lis_toolbag/c3runtime/domSide.js
'use strict';
{
  const DOM_COMPONENT_ID = 'lis_toolbag';
  ...
  const HANDLER_CLASS = class extends self.DOMHandler {
    constructor(iRuntime) {
      ...
      this.AddRuntimeMessageHandler(
        'disable-audio-mute',
        this.MuteGame.bind(this),
      );

      this.AddRuntimeMessageHandler(
        'disable-audio-unmute',
        this.UnmuteGame.bind(this),
      );
    }

    GetAudioHandler() {
      const iRuntime = this.GetRuntimeInterface();
      return iRuntime._domHandlers.find((handler) =>
        handler.GetComponentID() === "audio"
      );
    }

    MuteGame() {
      const audioDomHandler = this.GetAudioHandler();
      if (!audioDomHandler) return;

      audioDomHandler._muteGame = true;

      if (!this._audioHacked) {
        this._audioHacked = true;

        this._muteAudioInstances = () => {
          for (const instance of audioDomHandler._audioInstances) {
            instance._gainNode["gain"]["value"] = 0;

            const prototype = Object.getPrototypeOf(instance);
            if (prototype.__volumeDecorated) continue;
            prototype.__volumeDecorated = true;

            const SetVolume = prototype.SetVolume;
            prototype.SetVolume = function (v) {
              SetVolume.call(this, v);
              if (this._audioDomHandler._muteGame) {
        ...
        const _Play = audioDomHandler._Play;
        audioDomHandler._Play = (e) => {
          return _Play.call(audioDomHandler, e).then(() => {
            if (audioDomHandler._muteGame) {
              this._muteAudioInstances();
            }
          });
        };
    ...
    UnmuteGame() {
      ...
      audioDomHandler._muteGame = false;

      for (const instance of audioDomHandler._audioInstances) {
        instance._gainNode["gain"]["value"] = instance.GetOutputVolume();
      }
    }
  };
...
/** @import { SDK as YSDK } from "@types/ysdk" */
...
const HANDLER_CLASS = class extends self.DOMHandler {
  constructor(iRuntime) {
    ...
    this.AddRuntimeMessageHandler("start-tv-remote-tracking", () => {
      ...
      window.addEventListener("keydown", (event) => {
        if (event["key"] === "Enter")
          this.PostToRuntime("tv-remote-ok-click", true);
      });

      window.addEventListener("keyup", (event) => {
        if (event["key"] === "Enter")
          this.PostToRuntime("tv-remote-ok-click", false);
      });
    });
  ...
  GamepadsUpdate() {
    ...
    for (const gamepad of gamepads) {
      ...
      shouldUpdate = true;
      this.data["upPressed"] =
        this.data["upPressed"] || gamepad.buttons[12].pressed;
      this.data["downPressed"] =
        this.data["downPressed"] || gamepad.buttons[13].pressed;
      this.data["leftPressed"] =
        this.data["leftPressed"] || gamepad.buttons[14].pressed;
      this.data["rightPressed"] =
        this.data["rightPressed"] || gamepad.buttons[15].pressed;
    }
...
class YandexGamesSDKHandler {
  constructor(domHandler) {
    ...
    console.log(
      "%c YandexGamesSDK for Construct 3 v3.3.1 ",
      "background: #14151f; color: #fb923c"
    );

    /** @type {import("@lis-sdk/js").SDK} */
    this.sdk = undefined;

    // Note: For direct usage in JS scripts
    // Note: Prevents 'sdk is not defined' error
    window["sdk"] = undefined;
    window.sdk = undefined;

    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-initialize",
      this.InitializeYSDK.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-loading-api-ready",
      this.YSDKLoadingAPIReady.bind(this)
    );
    ...
    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-gameplay-api-stop",
      this.YSDKGameplayAPIStop.bind(this)
    );
    ...
    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-show-rewarded-ad",
      this.YSDKShowRewardedAD.bind(this)
    );

    this.domHandler.PostToRuntime("window-size-update", {
    ...
    let resizeBannerTimeout = null;
    window.addEventListener("resize", () => {
      if (resizeBannerTimeout) {
        clearTimeout(resizeBannerTimeout);
        resizeBannerTimeout = null;
      }

      resizeBannerTimeout = setTimeout(() => {
        for (const [key] of this.bannerMap) {
          const [tag, position] = key.split("-");
          this.ResizeAdaptiveBanner(tag, position);
        }
    ...
    this.bannersRoot = document.createElement("div");
    this.bannersRoot.style.position = "absolute";
    this.bannersRoot.style.left = "0";
    this.bannersRoot.style.top = "0";
    document.body.appendChild(this.bannersRoot);

    this.domHandler.AddRuntimeMessageHandler(
      "show-adaptive-banner",
      this.ShowAdaptiveBanner.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "hide-adaptive-banner",
      this.HideAdaptiveBanner.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "hide-adaptive-banner-all",
      this.HideAdaptiveBannerAll.bind(this)
    );
    ...
    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-purchase",
      this.YSDKPurchase.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-consume-purchase",
      this.YSDKConsumePurchase.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-get-purchases",
      this.YSDKGetPurchases.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-get-catalog",
      this.YSDKGetCatalog.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-get-player",
      this.YSDKGetPlayer.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-get-player-data",
      this.YSDKGetPlayerData.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-get-player-stats",
      this.YSDKGetPlayerStats.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-set-player-data",
      this.YSDKSetPlayerData.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-set-player-stats",
      this.YSDKSetPlayerStats.bind(this)
    );
    ...
    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-dispatch-event",
      this.YSDKDispatchEvent.bind(this)
    );
    ...
    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-request-review",
      this.YSDKRequestReview.bind(this)
    );
    ...
    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-get-all-games",
      this.YSDKGetAllGames.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "ysdk-get-game-by-id",
      this.YSDKGetGameByID.bind(this)
    );

    this.domHandler.AddRuntimeMessageHandler(
      "crazy-games-happy-time",
      this.CrazyGamesHappyTime.bind(this)
    );
  }

  async InitializeYSDK(params) {
    /** @type {typeof import("@lis-sdk/js")} */
    const LisSDK = window["LisSDK"];

    if (!LisSDK) {
      throw new Error("LisSDK global is not defined");
    }

    const platform = LisSDK.detectPlatform();

    const plugins = [];

    const AdFoxEnabled = params.properties[2];
    const AdFoxAdConsole = params.properties[3];

    if (AdFoxEnabled && !params.isPreview && platform === "playdeck") {
      let AdFoxFullscreenDesktopProps;
      let AdFoxFullscreenMobileProps;

      if (params.properties[4].length > 0) {
        AdFoxFullscreenDesktopProps = JSON.parse(params.properties[4]);
      }

      if (params.properties[5].length > 0) {
        AdFoxFullscreenMobileProps = JSON.parse(params.properties[5]);
      }

      if (AdFoxFullscreenDesktopProps || AdFoxFullscreenMobileProps) {
        plugins.push(
          LisSDK.AdfoxPlugin({
            adConsole: AdFoxAdConsole,
            fullscreenDesktop: AdFoxFullscreenDesktopProps,
            fullscreenMobile: AdFoxFullscreenMobileProps,
          })
        );
      }
    }

    const GameAnalyticsEnabled = params.properties[6];

    if (GameAnalyticsEnabled) {
      const GameAnalyticsGameKey = params.properties[7];

      if (GameAnalyticsGameKey.length !== 32) {
        console.warn("GameAnalytics game key must be 32 characters long");
      }

      const GameAnalyticsSecretKey = params.properties[8];

      if (GameAnalyticsSecretKey.length !== 40) {
        console.warn("GameAnalytics secret key must be 40 characters long");
      }

      plugins.push(
        LisSDK.GameAnalyticsPlugin({
          logging: true,
          eventSubmission: !params.isPreview,
          gameKey: GameAnalyticsGameKey,
          secretKey: GameAnalyticsSecretKey,
        })
      );
    }

    this.sdk = await LisSDK["init"]({ plugins });

    // Note: For direct usage in JS scripts
    window["sdk"] = this.sdk;
    window.sdk = this.sdk;

    await this.OnYSDKLoaded();

    /** @type {Partial<InitializationData>} */
    const data = {};

    data.platformName = this.sdk["platform"]["name"];

    if (this.sdk["platform"]["name"] === "yandex_games") {
      /** @type {YSDK} */
      const ysdk = this.sdk["platform"]["sdk"];

      data.appID = ysdk["environment"]["app"]["id"];
      data.domain = ysdk["environment"]["browser"]["lang"];
      data.payload = ysdk["environment"]["payload"];
      data.developerURL = (await ysdk["features"]["GamesAPI"]["getAllGames"]())[
        "developerURL"
      ];
    }

    data.playerLanguage = this.sdk["player"]["language"];
    data.deviceType = this.sdk["player"]["device"];
    data.fullscreenADSupported = this.sdk["adv"]["interstitial"]["supported"];
    data.rewardedADSupported = this.sdk["adv"]["rewarded"]["supported"];
    data.adaptiveBannerSupported =
      this.sdk["adv"]["banners"]["adaptive"]["supported"];
    data.adaptiveBannerSizes = this.sdk["adv"]["banners"]["adaptive"]["sizes"];
  ...
  async OnYSDKLoaded(){return;
    if (this.sdk["platform"]["name"] === "yandex_games") {
      ...
      ysdk["on"]("game_api_resume", () => {
        this.domHandler.PostToRuntime("ysdk-handle-event", {
          ["type"]: "game_api_resume",
        });
      });

      await Promise.all([
        this.YSDKUpdateCanShowShortcutPrompt(),
        this.YSDKUpdateCanReview(),
      ]);
    }
  }

  YSDKLoadingAPIReady() {
    this.sdk["metrics"]["gameIsReady"]();
  }

  YSDKGameplayAPIStart() {
    this.sdk["metrics"]["gameplay"]["start"]();
  }

  YSDKGameplayAPIStop() {
    this.sdk["metrics"]["gameplay"]["stop"]();
  }

  YSDKShowFullscreenAD() {
    if (!this.sdk["adv"]["interstitial"]["supported"]) return;

    this.sdk["adv"]["interstitial"]["show"]({
      ["onOpen"]: () => {
  ...
  YSDKShowRewardedAD(params) {
    if (!this.sdk["adv"]["rewarded"]["supported"]) return;

    this.sdk["adv"]["rewarded"]["show"]({
      ["onOpen"]: () => {
  ...
  CalculateBannerRect(rect, position) {
  ...
  GetAdaptiveBannerContainer(tag, position) {
    const key = `${tag}-${position}`;

    if (this.bannerMap.has(key)) {
      return this.bannerMap.get(key);
    }

    const container = document.createElement("div");
    container.style.position = "absolute";
    container.id = `adaptive-banner-${tag}-${position}`;
    this.bannersRoot.appendChild(container);
    this.bannerMap.set(key, container);

    return container;
  }

  DestroyAdaptiveBannerContainer(tag, position) {
    const key = `${tag}-${position}`;

    const container = this.bannerMap.get(key);
    if (!container) return;

    this.bannersRoot.removeChild(container);
    this.bannerMap.delete(key);
  }

  ResizeAdaptiveBanner(tag, position) {
    const element = document.querySelector(
      `[data-adaptive-banner-tag="${tag}"]`
    );
    ...
    const bannerRect = this.CalculateBannerRect(
      element.getBoundingClientRect(),
      position
    );

    const container = this.GetAdaptiveBannerContainer(tag, position);

    container.style.left = `${bannerRect.left}px`;
    container.style.top = `${bannerRect.top}px`;
    container.style.width = `${bannerRect.width}px`;
    container.style.height = `${bannerRect.height}px`;
  }

  ShowAdaptiveBanner(params) {
    if (!this.sdk.adv.banners.adaptive.supported) return;
    ...
    const container = this.GetAdaptiveBannerContainer(tag, position);
    this.ResizeAdaptiveBanner(tag, position);
    this.sdk.adv.banners.adaptive.show(container.id);
  }

  HideAdaptiveBanner(params) {
    if (!this.sdk.adv.banners.adaptive.supported) return;
    ...
    const container = this.GetAdaptiveBannerContainer(tag, position);
    this.sdk.adv.banners.adaptive.hide(container);
    this.DestroyAdaptiveBannerContainer(container.id);
  }

  HideAdaptiveBannerAll() {
    if (!this.sdk.adv.banners.adaptive.supported) return;

    this.sdk.adv.banners.adaptive.hideAll();
  }

  YSDKShowStickyBanner() {
    if (!this.sdk["adv"]["banners"]["static"]["supported"]) return;

    this.sdk["adv"]["banners"]["static"]["show"]();
  }

  YSDKHideStickyBanner() {
    if (!this.sdk["adv"]["banners"]["static"]["supported"]) return;

    this.sdk["adv"]["banners"]["static"]["hide"]();
  }
  ...
  async YSDKGetLeaderboardEntries(params) {
    if (this.sdk["platform"]["name"] === "yandex_games") {
      ...
      if (params["options"]["includeUser"]) {
        const loggedIn = await ysdk.isAvailableMethod(
      ...
      const data = await lb["getLeaderboardEntries"](
      ...
      const bottomStart = Math.max(
        ...
        Math.min(
          data["entries"].findIndex(
            (entry) => entry["rank"] === data["userRank"]
          ),
      ...
      ranges.push({
        start: ranges[0].length - 1,
        length: entries.length - ranges[0].length,
      });
  ...
  async YSDKGetPlayerData(keys) {
    const value = await this.sdk.storage.getItem("data");
  ...
  async YSDKGetPlayerStats(keys) {
    const value = await this.sdk["storage"]["getItem"]("stats");

    let stats;

    try {
      stats = value ? JSON.parse(value) : {};
    } catch {
      stats = {};
    }
  ...
  async YSDKSetPlayerData(params) {
    ...
    await this.sdk["storage"]["setItem"]("data", JSON.stringify(data));
  }

  async YSDKSetPlayerStats(_stats) {
    const value = await this.sdk["storage"]["getItem"]("stats");
    ...
    await this.sdk["storage"]["setItem"]("stats", JSON.stringify(stats));
  }

  async YSDKIncrementPlayerStats(_stats) {
    const value = await this.sdk["storage"]["getItem"]("stats");
    ...
    Object.entries(_stats).forEach(([key, number]) => {
  ...
  async YSDKGetPlayer(params) {
    if (this.sdk["platform"]["name"] === "yandex_games") {
      ...
      if (params["requestPersonalInfo"] && ysdk["openAuthDialog"]) {
  ...
  YSDKDispatchEvent(params) {
    if (this.sdk["platform"]["name"] === "yandex_games") {
      ...
      ysdk["dispatchEvent"](ysdk["EVENTS"][params["name"]]);
    }
  }
  ...
  async YSDKUpdateCanShowShortcutPrompt() {
    if (this.sdk["platform"]["name"] === "yandex_games") {
      ...
      await this.domHandler.PostToRuntimeAsync(
        "ysdk-update-can-show-shortcut-prompt",
        {
  ...
  async YSDKGetGameByID(params) {
    if (this.sdk["platform"]["name"] === "yandex_games") {
      ...
      const result = await ysdk["features"]["GamesAPI"]["getGameByID"](
        params["id"]
      );
  ...
  async YSDKRemoteConfigFetch(params) {
    if (this.sdk["platform"]["name"] === "yandex_games") {
      ...
      const config = await ysdk["getFlags"](params);
  ...
  YSDKServerTimeUpdate() {
    ...
    this.domHandler.PostToRuntime(
      "ysdk-server-time-update",
      this.sdk["time"]()
    );
  }

  CrazyGamesHappyTime() {
    if (this.sdk["platform"]["name"] === "crazy_games") {
      const SDK = this.sdk["platform"]["sdk"];
...
class TVRemoteEmulator {
  ...
  OnMessage(event) {
    ...
    switch (event["data"]["name"]) {
      ...
      default: {
        const button =
          this.fakeController.buttons[this.ArrowMap[event["data"]["name"]]];
