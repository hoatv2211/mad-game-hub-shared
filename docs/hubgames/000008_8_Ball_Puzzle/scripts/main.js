...
"_system");else if(exportType==="preview"||this._iRuntime.IsWebView2Wrapper()){}else if(!this._isConstructArcade)if(target===2)window.top.location=url;else if(target===1)window.parent.location=url;else window.location=url}else if(type==="new-window"){const url=e["url"];const tag=e["tag"];if(self["cordova"]&&self["cordova"]["InAppBrowser"])self["cordova"]["InAppBrowser"]["open"](url,"_system");else {}}}_OnRequestFullscreen(e){if(this._iRuntime.IsWebView2Wrapper()||
